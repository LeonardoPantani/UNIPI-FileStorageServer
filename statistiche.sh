...
# salvo le variabili
read -r -u 4 data avg_read avg_written n_read n_write n_lock n_openlock n_unlock n_delete n_close max_size_reached max_file_number_reached n_replace_applied max_concurrent_connections
...
echo -e "READ MEDIA\tWRITE MEDIA\t#READ\t#WRITE\t#LOCK\t#OPEN_LOCK\t#UNLOCK\t#DELETE\tCLOSE"
echo -e "$avg_read bytes\t$avg_written bytes\t$n_read\t$n_write\t$n_lock\t$n_openlock\t\t$n_unlock\t$n_delete\t$n_close"
echo ''
echo -e "DIM. MAX OCCUPATA\t#FILE MAX\t#APPLICAZIONI ALGORITMO DI ESPULSIONE\t#CONNESSIONI CONCORRENTI"
echo -e "$max_size_reached MB\t\t$max_file_number_reached\t\t$n_replace_applied\t\t\t\t\t$max_concurrent_connections"
echo ''
