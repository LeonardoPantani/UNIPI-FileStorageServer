            ...
            if(el != NULL) {
                if(el->lock == -1 || el->lock == socketConnection) { // il lock non c'è o è già settato dal client che ha fatto richiesta
                    ...
                    while(TRUE) {
                        ...
                        printSave("GC %d > Svegliato dall'attesa per la lock!", numero);
                    ...
                    printSave("GC %d > Uscito dal while attesa lock.", numero);
            ...
            if(el != NULL) {
                if(el->lock == socketConnection) {
                    if(el->codaRichiedentiLock == NULL) {
                        ...
                        printSave("GC %d > Lock assegnata al primo in coda.", numero);
                        if(el->codaRichiedentiLock->numClients == 0) {
                            deleteQueue(el->codaRichiedentiLock);
                            el->codaRichiedentiLock = NULL;
                            ppf(CLR_INFO); printSave("GC %d > Ho eliminato la coda visto che il client estratto era l'ultimo", numero); ppff();
                        }

                        if(el->rilascioLock != NULL) {
                            if(pthread_cond_broadcast(el->rilascioLock) == 0) {
                                ppf(CLR_ERROR); printSave("GC %d > Errore broadcast lock rilasciato!!", numero); ppff();
                            }
                        } else {
                            ppf(CLR_ERROR); printSave("GC %d > Variabile di condizione lock inesistente!!", numero); ppff();
                        }
                    }
